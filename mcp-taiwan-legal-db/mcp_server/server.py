...
async def search_judgments(
    ...
    Args:
        ...
        month_from/day_from: 起始月／日（民國曆；搭配 year_from 用於細粒度窗口）
        month_to/day_to: 截止月／日（民國曆；搭配 year_to 用於細粒度窗口）
        case_word: 字別（如「台上」「上易」「重訴」），查特定案號時必填
        case_number: 案號（數字），查特定案號時必填
        main_text: 裁判主文關鍵字（對應 jud_jmain）— 結構化篩選輸贏方
        max_results: 回傳筆數上限（預設 10，上限 500 — 司法院網站對單次查詢結果的硬上限）

    Returns:
        包含搜尋結果的字典：success, query, total_count, results, cached, timestamp
    """
    # 司法院網站對單次查詢硬性限制「只顯示前 500 筆」；超過此數值無意義。
    # 單輪窮盡搜尋由呼叫端透過 month/day 切窗口實現。
    max_results = min(max_results, 500)
    logger.info("search_judgments: keyword=%r, court=%r, case_type=%r, "
                "year=%s~%s, month=%s~%s, day=%s~%s, "
                "case_word=%r, case_number=%r, main_text=%r",
