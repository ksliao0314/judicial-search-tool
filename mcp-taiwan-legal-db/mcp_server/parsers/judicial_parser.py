...
def _enrich_from_jid(entry: dict) -> None:
    ...
    若 JID prefix 對照表查不到（如 2022 年行政訴訟新制後的 TPTA/TCTA/KSTA
    之類新代碼尚未納入 COURT_CODES），fallback 從 entry["case_id"] 的中文
    描述擷取法院名（case_id 格式：「法院全名 [分院/地方庭] 年度 字 字第 N 號…」）。
    """
    jid = entry.get("jid", "")
    if not jid:
        _fallback_court_from_case_id(entry)
        return
    ...
    # 若對照表未命中（新代碼 / 改制後代碼），從 case_id 擷取法院名稱
    if not entry.get("court"):
        _fallback_court_from_case_id(entry)


# case_id 開頭到第一個數字前的中文/空白段視為法院名稱
# 例：「臺北高等行政法院 地方庭 113 年度 交 字第 1138 號判決」→「臺北高等行政法院 地方庭」
_CASE_ID_COURT_RE = re.compile(r"^([\u4e00-\u9fff\s]+?)\s*\d")


def _fallback_court_from_case_id(entry: dict) -> None:
    """從 case_id 中文描述擷取法院名；JID 對照表未命中時使用。"""
    case_id = entry.get("case_id", "")
    if not case_id:
        return
    m = _CASE_ID_COURT_RE.match(case_id)
    if m:
        court = m.group(1).strip()
        # 內部多個空白壓縮為單一空白（"臺北高等行政法院  地方庭" → "臺北高等行政法院 地方庭"）
        entry["court"] = re.sub(r"\s+", " ", court)
...
# Outline marker break pattern：
# 最高行政法院（TPAA）等用 .htmlcontent 的 "Word-paste" 格式，每段一個 <div>，
# 但段落內部的 (一)(二) / 1.2. / ㈠㈡ 等 outline marker 完全沒有換行符 →
# 整段塞成 1000+ 字一行，frontend 解析器靠「行首」偵測 marker 全部失效。
#
# 對策：在「句末標點 + outline marker」之間強制插入 \n，模擬其他法院 .text-pre
# 已有的硬斷行。對 .text-pre 來源無 side effect（marker 前已有 \n，regex 不命中）。
# Pattern tolerance aligned with frontend `app.js` `_detectOutlineLevel` regexes：
#  - L0 / L1 後接標點接受 `、，.．` 四種（舊判決 / 手打格式常用 `.` 或 `．`）
#  - L1 / L2 paren 的 CJK 數字接受 `百零〇`（罕見但存在，如 `一百零一、`）
#  - L3 arabic 接受全形數字 `\uFF10-\uFF19` 與全形句點 `．`
#  - L3 arabic 標點容忍度擴到 `[.．]`（不含 `、`，避免把 `1、` 誤當 L3；
#    真 L3 寫 `1、...` 的 case 極罕）
#  - L4 paren 同上，接受全形數字
# 對 .text-pre 來源無 side effect（marker 前已有 \n，regex 不命中）。
_OUTLINE_BREAK_PATTERNS = [
    # L0：壹貳參…甲乙…後接 、，.．
    re.compile(r'([。：；])(?=[壹貳參肆伍陸柒捌玖拾甲乙丙丁戊己庚辛壬癸]+[、，.．])'),
    # L1：一、二、… 含 百零〇，後接 、，.．
    re.compile(r'([。：；])(?=[一二三四五六七八九十百零〇]{1,4}[、，.．])'),
    # L2 unicode ㈠–㈩
    re.compile(r'([。：；])(?=[\u3220-\u3229])'),
    # L2 paren （一）/(一)，含 百零〇
    re.compile(r'([。：；])(?=[（(][一二三四五六七八九十百零〇]{1,3}[）)])'),
    # L3 unicode ⒈–⒛
    re.compile(r'([。：；])(?=[\u2488-\u249B])'),
    # L3 arabic 1. 2. (含全形 １．)，後接非數字（避免 1.5 / 1.234）
    re.compile(r'([。：；])(?=[\d\uFF10-\uFF19]{1,2}[.．](?![\d\uFF10-\uFF19]))'),
    # L4 unicode ⑴–⒇
    re.compile(r'([。：；])(?=[\u2474-\u2487])'),
    # L4 paren (1)(2)，含全形數字
    re.compile(r'([。：；])(?=[（(][\d\uFF10-\uFF19]{1,2}[）)])'),
    # L5 unicode ①–⑳
    re.compile(r'([。：；])(?=[\u2460-\u2473])'),
]


def _insert_outline_breaks(text: str) -> str:
    """在句末標點 + outline marker 之間插入 \\n，恢復段落結構。

    主要解決最高行政法院（.htmlcontent 來源）段落沒硬斷行的問題。
    """
    for pat in _OUTLINE_BREAK_PATTERNS:
        text = pat.sub(r'\1\n', text)
    return text
...
def parse_judgment_page(html: str, jid: str | None = None) -> dict:
    ...
    if content_el:
        ...
        full_text = _insert_outline_breaks(full_text)
...
def _extract_main_sections(result: dict, lines: list[str]):
    ...
    for line in lines:
        ...
        if normalized == "主文":
            current_section = "主文"
            continue
        # 純事實段（民/行訴的事實段、刑事的犯罪事實段）→ 歸入 facts
        elif normalized in ("事實", "犯罪事實"):
            current_section = "事實"
            continue
        # 合併段（簡易判決常見「事實及理由」等）→ 歸入 reasoning
        # 因為合併段的實質內容以法院論述為主（適用法律、認定理由、判決依據）
        # 若誤歸為 facts 會導致下游 reasoning 為空，且過去 fallback 複製到 reasoning 會造成重複資料
        elif normalized in ("事實及理由", "事實與理由", "犯罪事實及理由"):
            current_section = "理由"
            continue
        elif normalized == "理由":
            current_section = "理由"
    ...
    # Fallback：罕見的未知標題格式導致 reasoning 為空、但 full_text 有內容
    # 理論上「事實及理由」等合併段已歸入 reasoning，但保留此安全網涵蓋其他異常結構
    if not result["reasoning"] and result.get("full_text"):
        # 如果 facts 有大量內容（> 200 字）且 reasoning 空，可能是未知合併標題
        # → 搬移（不是複製）facts 到 reasoning，避免兩欄重複資料
        if len(result["facts"]) > 200:
            result["reasoning"] = result["facts"]
            result["facts"] = ""
        else:
            # 否則用 full_text（去掉已解析的 main_text 部分）
            ft = result["full_text"]
            mt = result["main_text"]
            if mt and mt in ft:
                # full_text 中 main_text 之後的部分就是理由
                idx = ft.index(mt) + len(mt)
                result["reasoning"] = ft[idx:].strip()
            else:
                result["reasoning"] = ft
