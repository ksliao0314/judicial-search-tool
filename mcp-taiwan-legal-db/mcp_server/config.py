...
# Judgment TTL 從 30 天下修到 7 天：律師研究工作中，parser 調整/修正的迭代週期
# 通常在 1-2 週以內，過長 TTL 會讓舊版解析結果（例如 reasoning 分段失敗的版本）
# 持續被取用。搜尋結果 TTL 保持 24 小時；法規不變動所以 7 天 OK。
CACHE_JUDGMENT_TTL = 604800    # 7 天
CACHE_SEARCH_TTL = 86400       # 24 小時
CACHE_REGULATION_TTL = 604800  # 7 天
CACHE_PCODE_TTL = 2592000      # 30 天

# 搜尋限速：上游 upstream 已移除 Playwright 改用 httpx（加 F5 WAF bypass cookie
# 策略）。httpx 路徑比 Playwright 快很多，但我們的客製 0.3-1s delay 是針對
# Playwright 實測的節奏，對 httpx 可能過猛觸發司法院 WAF 限速。
# 先採保守中間值：1s min、2s max（比上游新 1-3s 略緊、比我們原本 0.3-1s 寬），
# 觀察一週無 block 後再視需要調低。
SEARCH_RATE_LIMIT = 10         # 每分鐘最多 10 次（httpx 比 Playwright 快、值可維持）
SEARCH_DELAY_MIN = 1.0         # 秒
SEARCH_DELAY_MAX = 2.0         # 秒
