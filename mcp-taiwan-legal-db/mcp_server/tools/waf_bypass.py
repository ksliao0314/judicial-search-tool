...
async def get_with_waf_retry(
    ...
    兩條 block 偵測路徑：
    1. body-level：HTTP 200 但 body 含 "Request Rejected" / bobcmn JS 挑戰 → is_blocked=True
    ...
    except (_httpx.ReadError, _httpx.ConnectError, _httpx.RemoteProtocolError) as exc:
        logger.info("WAF bypass: network-level error (%s)、嘗試 refresh cookies 後重試：%s",
