...
class JudicialSearchClient:
    ...
    async def search(
        ...
        month_from/day_from/month_to/day_to 對應進階搜尋的 dm1/dd1/dm2/dd2
        欄位（ROC 曆）。窮盡搜尋超過 500 筆硬上限時，呼叫端用這些做 keyset
        pagination 切窗口。
        """
        params = {
            "keyword": keyword,
    ...
    async def _keyword_search_http(
        ...
        async with httpx.AsyncClient(
            ...
            # 月 / 日 細粒度窗口：供窮盡搜尋切片用（超過 500 筆硬上限時）
            if params.get("month_from"):
                form_data["dm1"] = str(params["month_from"])
            if params.get("day_from"):
                form_data["dd1"] = str(params["day_from"])
            if params.get("month_to"):
                form_data["dm2"] = str(params["month_to"])
            if params.get("day_to"):
                form_data["dd2"] = str(params["day_to"])
