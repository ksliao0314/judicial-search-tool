...
> 給律師用的台灣司法院判決 / 憲法解釋 / 勞動部訴願 AI 篩選工具。  
> 律師輸入關鍵字，系統去對應來源抓判決 / 解釋 / 訴願決定、AI 逐筆讀完、給每筆 0-10 分與立場標註，律師專注在「法院 / 訴願會的判斷」上。
...
- 搜尋頁驗證碼 → fetch 圖丟 **Claude Haiku vision** 讀 5 碼（重用 app 既有 Claude client + `MODEL_SCORING`）、解錯換一張重試；**翻頁帶 `pageNumber` + 接力 token、免再解**（一次搜尋只解一張）。
- 詳情頁 `/Appeal/AppealCaseDecisionContent?caseId=N` **公開無驗證碼**、並行抓。`parse_detail` 切 主文 / 事實(導言) / 理由 / 全文。
- **fetch 一律靠 `source_url` 分流**：hit 的 `case_id` 是訴願字號（非 FJUD jid），`filter._fetch_one(jid, source_url=)` 偵測含 `appealweb.mol.gov.tw` → `get_appeal_decision`。stage3 / stage25 / prefilter / retry 四條 fetch 路徑 + reader Stage 2 都要傳 source_url（漏傳會把字號當 FJUD jid 打 MCP 全失敗）。
