...
**特別強調 — 構成要件 / 實質爭點認定段**：
  - 刑事：法院對**主觀故意、不法意圖、客觀行為、詐術、陷於錯誤、交付財物、因果關係**等要件如何論證
  - 民事：法院對**請求權要件、侵權行為要件、違約要件**等要件如何論證
  - 行政：法院對**違法性、裁量濫用、比例原則、法律保留**等要件如何論證
  這類「實質法律認定」段是律師最在意的內容、應優先 excerpt。
...
**絕對禁止來源**（三層規則都必須遵守、即使該段文字看起來很命中）：
  ...
  - **刑事判決的「論罪」「論罪科刑」「罪數認定」「量刑」「執行」段落** —
    這是認定構成要件成立**之後**的程序性決定（罪名適用、罪數計算、刑度裁量、
    緩刑、執行方式等）、跟「法院如何認定構成要件」核心問題無關。
    典型語句（出現在 excerpt 就屬誤選）：
    「核被告 X 所為，均係犯刑法第 Y 條」「應依刑法第 Y 條論處」
    「應以一罪論」「應以一罪論之」「數罪併罰」「爰以行為人之責任…」
    「從一重論處」「論以…一罪」
...
clusters 規則（**律師看到的真正分類就在這裡，比 direction 重要**）：
  ...
  - 若無法有意義地分群 → 填 []
  格式：[{{"label":"10字內","case_ids":["id1","id2",...]}}]

  **硬規則**：
    1. cluster 總數上限 **7**（含「其他」）；理想 3-6 群。
    2. 每群大小不限（一群可以 1 筆、也可以 20 筆，由主題內容決定、不必均等）。
    3. 盡量把每一筆相關判決歸入某個主題 cluster；剩下無法歸類者才放入「其他」。
       「其他」**必要時才建立**，若主題 cluster 已涵蓋全部判決、不需要「其他」。
    4. 同一筆判決若主題跨多類、可重複出現在多群。
...
# prompt 中最多帶幾筆（top score）；cluster 要覆蓋所有相關判決、所以調高。
# 超過此上限的 case 不會進 LLM 視野、無法被 cluster 歸群、僅出現在「全部」tab。
# 最大值考量：input tokens (80 筆 ≈ 10K) + output tokens (80 case_ids ≈ 2K) + summary 1K 預算
_SYNTHESIS_MAX_ITEMS = 80
...
_MAX_CLUSTERS = 7  # cluster 總數硬上限（含「其他」）


def _enforce_cluster_coverage(
    clusters: list[dict],
    relevant_case_ids: list[str] | None,
) -> list[dict]:
    """Cluster 後處理：截斷 > 7、補「其他」cluster 涵蓋 orphan case。

    規則：
    1. 若 cluster 數 > _MAX_CLUSTERS → 按 case_ids 數排序、保留前 6、其餘合併進「其他」。
    2. 若 relevant_case_ids 提供、偵測 orphans（relevant 但不在任何 cluster case_ids 內）
       → 附加到「其他」（若存在）或新建「其他」cluster（但總數不得超過上限）。

    Fuzzy match：case_id A ∈ cluster.case_ids 若 A==cid / A in cid / cid in A、
    與 FE countClusterMatches 邏輯一致。
    """
    if not isinstance(clusters, list):
        clusters = []

    # 規整：去除無效 item、確保 case_ids 是 list of str
    valid_clusters = []
    for c in clusters:
        if not isinstance(c, dict):
            continue
        cids = c.get("case_ids") or []
        if not isinstance(cids, list):
            cids = []
        cids = [str(x) for x in cids if isinstance(x, (str, int))]
        label = str(c.get("label") or "").strip()
        if not label:
            continue
        valid_clusters.append({"label": label, "case_ids": cids})

    # Step 1：截斷 > 7
    if len(valid_clusters) > _MAX_CLUSTERS:
        valid_clusters.sort(key=lambda c: len(c["case_ids"]), reverse=True)
        kept = valid_clusters[: _MAX_CLUSTERS - 1]
        overflow = valid_clusters[_MAX_CLUSTERS - 1:]
        merged_ids: list[str] = []
        for c in overflow:
            merged_ids.extend(c["case_ids"])
        # 找既有「其他」cluster 或新建一個
        other_idx = next(
            (i for i, c in enumerate(kept) if c["label"] == "其他"), None
        )
        if other_idx is not None:
            # 合併 overflow 到既有「其他」（去重）
            existing = set(kept[other_idx]["case_ids"])
            kept[other_idx]["case_ids"] += [x for x in merged_ids if x not in existing]
        elif merged_ids:
            kept.append({"label": "其他", "case_ids": list(dict.fromkeys(merged_ids))})
        valid_clusters = kept

    # Step 2：偵測 orphans、補進「其他」
    if relevant_case_ids:
        covered: set[str] = set()
        for c in valid_clusters:
            for cid in c["case_ids"]:
                # fuzzy match（與 FE 一致）：把所有 relevant 中「與任一 cluster cid 有包含關係者」視為 covered
                for rid in relevant_case_ids:
                    if rid == cid or cid in rid or rid in cid:
                        covered.add(rid)
        orphans = [rid for rid in relevant_case_ids if rid not in covered]
        if orphans:
            other_idx = next(
                (i for i, c in enumerate(valid_clusters) if c["label"] == "其他"), None
            )
            if other_idx is not None:
                existing = set(valid_clusters[other_idx]["case_ids"])
                valid_clusters[other_idx]["case_ids"] += [x for x in orphans if x not in existing]
            elif len(valid_clusters) < _MAX_CLUSTERS:
                valid_clusters.append({"label": "其他", "case_ids": orphans})
            else:
                # 已滿 7 cluster 仍有 orphan（極罕見）→ 把 orphan 塞進最小的 cluster
                smallest = min(valid_clusters, key=lambda c: len(c["case_ids"]))
                existing = set(smallest["case_ids"])
                smallest["case_ids"] += [x for x in orphans if x not in existing]

    return valid_clusters


def _validate_synthesis(data: object, relevant_case_ids: list[str] | None = None) -> dict:
    ...
    relevant_case_ids：所有 score>0 的 case_id list、用於 cluster coverage enforcement
    （_enforce_cluster_coverage 會據此截斷 > 7 cluster 並補「其他」填 orphan）。
    """
    ...
    clusters = _enforce_cluster_coverage(data.get("clusters") or [], relevant_case_ids)
...
async def run_synthesis(
    ...
    for attempt in range(1, _SYNTHESIS_MAX_RETRIES + 1):
        ...
        try:
            ...
            parsed = extract_json(raw_text)
            # 傳入所有相關 case_ids 讓 _validate 做 coverage 檢查（補「其他」填 orphan）
            all_relevant_ids = [r.get("case_id", "") for r in results if r.get("case_id")]
            synthesis = _validate_synthesis(parsed, relevant_case_ids=all_relevant_ids)
